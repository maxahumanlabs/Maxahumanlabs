      ...
      <section className="relative w-full bg-gradient-to-br from-[#f4f7fa] to-[#ffffff] overflow-hidden min-h-[600px] flex items-center pt-24 pb-20 md:py-32">
        ...
        <div className="px-6 sm:px-8 md:px-12 lg:px-12 xl:px-12 max-w-[1400px] mx-auto w-full flex flex-col md:flex-row relative z-20">
          ...
          <div className={`w-full md:w-[48%] flex flex-col items-center md:items-start text-center ${language === 'ar' ? 'md:text-right md:items-end md:ml-auto' : 'md:text-left'}`}>
            ...
            {/* WhatsApp Box */}
            <div className={`bg-white rounded-xl shadow-[0_2px_15px_rgb(0,0,0,0.04)] py-2 px-3 sm:pr-4 sm:pl-3 flex flex-col sm:flex-row items-center gap-4 mb-8 border border-gray-100 w-full max-w-[550px] ${language === 'ar' ? 'border-r-[5px] border-r-[#25D366]' : 'border-l-[5px] border-l-[#25D366]'}`}>
              <div className="w-11 h-11 bg-[#25D366] rounded-full flex items-center justify-center text-white shrink-0">
              ...
              <div className={`flex-1 text-center sm:text-left ${language === 'ar' ? 'sm:text-right' : ''}`}>
                 <p className="font-bold text-[#09031c] text-base md:text-[17px] tracking-tight mb-0.5">{t('hero_static.need_guidance')}</p>
                 <p className="text-xs sm:text-[13px] text-gray-500">{t('hero_static.chat_directly')}</p>
              </div>
              <a href={`https://wa.me/971506869550?text=${encodeURIComponent(t('hero_static.whatsapp_message'))}`} target="_blank" rel="noopener noreferrer" className="bg-[#0a0521] text-white px-6 py-2.5 rounded-full text-[13px] sm:text-sm font-medium hover:bg-black transition-colors shrink-0 whitespace-nowrap w-full sm:w-auto text-center">
