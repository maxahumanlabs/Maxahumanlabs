      ...
      <section className="relative w-full bg-[#f4f7fa] overflow-hidden min-h-0 md:min-h-[600px] flex flex-col md:flex-row items-center md:py-32 pt-0 pb-10 md:pb-20">
        ...
        <div className="px-6 sm:px-8 md:px-12 lg:px-12 xl:px-12 max-w-[1400px] mx-auto w-full flex flex-col md:flex-row relative z-20 pt-[270px] md:pt-0">
          ...
          <div className={`w-full md:w-[48%] flex flex-col items-center md:items-start text-center ${language === 'ar' ? 'md:text-right md:items-end md:ml-auto' : 'md:text-left'}`}>
            ...
            {/* Grid Features */}
            <div className="flex flex-col md:grid md:grid-cols-2 gap-y-2 md:gap-y-3 gap-x-8 text-[13px] sm:text-sm text-gray-500 mb-6 md:mb-8 font-medium order-5 md:order-4 items-center md:items-start w-full">
              {[5,6,1,2,3,4,7].map((i, index) => (
                <div key={i} className={`flex items-center gap-2 ${index > 1 ? 'hidden md:flex' : ''}`}>
                  {i === 5 ? (
                    ...
                    <svg className="w-4 h-4 shrink-0 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                  ) : (
